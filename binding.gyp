{
    "variables": {
        ...
        'soundtouchResampler': "<!(ls `pkg-config ttLibC --cflags-only-I | sed -e 's/\-I//g' | sed -e 's/ //g'`/ttLibC/resampler/soundtouchResampler.h 1>/dev/null 2>&1 && echo yes || echo no)",
    ...
    "targets": [
        ...
        {
            "conditions": [[
                'soundtouchResampler=="yes"', {
                    "defines": ["__ENABLE__"],
                    "include_dirs": [
                        "<!(pkg-config soundtouch --cflags-only-I | sed -e 's/\-I//g')"],
                    "libraries": [
                        "<!@(pkg-config --libs soundtouch)"]
                }
            ]],
            "target_name": "soundtouchResampler",
            "sources": [
                "src/c/resampler/soundtouchResampler.cpp",
                "src/c/frame/frame.cpp"],
            "include_dirs": [
                "<!(node -e \"require('nan')\")",
                "<!(pkg-config ttLibC --cflags-only-I | sed -e 's/\-I//g')"],
