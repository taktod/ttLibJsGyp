...
FdkaacEncoder::FdkaacEncoder(Local<Object> params) : Encoder() {
  type_ = get_fdkaac;
...
bool FdkaacEncoder::encodeCallback(ttLibC_Aac *aac) {
...
bool FdkaacEncoder::encode(ttLibC_Frame *frame) {
#ifdef __ENABLE_FDKAAC_ENCODE__
  // 中の準備ができてなかったら動作させない。
  if(handle_ == NULL) {
    puts("初期化されていません。");
  ...
  if(!is_pts_initialized_) {
    ...
    pts_ = frame->pts * sample_rate_ / frame->timebase;
  }
  uint8_t *data = (uint8_t *)frame->data;
  size_t left_size = frame->buffer_size;
	AACENC_ERROR err;
