...
class Openh264Encoder : public Nan::ObjectWrap {
public:
...
//        FramePassingWorker::Init();
        ...
        tpl->SetClassName(Nan::New("Openh264Encoder").ToLocalChecked());
        ...
        Nan::Set(
            target,
            Nan::New("Openh264Encoder").ToLocalChecked(),
            Nan::GetFunction(tpl).ToLocalChecked());
#endif
    }
#ifdef __ENABLE__
private:
    explicit Openh264Encoder(
            uint32_t width,
            uint32_t height,
            uint32_t maxQuantizer,
            uint32_t minQuantizer,
            uint32_t bitrate) {
        encoder_ = ttLibC_Openh264Encoder_make_ex(
                width,
                height,
                maxQuantizer,
                minQuantizer,
...
//        Nan::AsyncQueueWorker(new FramePassingWorker((ttLibC_Frame *)h264, callback));
    ...
    static NAN_METHOD(Encode) {
        ...
        Openh264Encoder *encoder = Nan::ObjectWrap::Unwrap<Openh264Encoder>(info.Holder());
        ...
        if(frame->type == frameType_yuv420) {
            if(!ttLibC_Openh264Encoder_encode(
                    encoder->encoder_,
                    (ttLibC_Yuv420 *)frame,
                    encodeCallback,
                    encoder)) {
...
NODE_MODULE(openh264Encoder, Openh264Encoder::Init);