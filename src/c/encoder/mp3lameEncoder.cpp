...
class Mp3lameEncoder : public Nan::ObjectWrap {
public:
    static NAN_MODULE_INIT(Init) {
#ifdef __ENABLE__
        Local<FunctionTemplate> tpl = Nan::New<FunctionTemplate>(New);
        tpl->SetClassName(Nan::New("Mp3lameEncoder").ToLocalChecked());
        ...
        Nan::Set(
            target,
            Nan::New("Mp3lameEncoder").ToLocalChecked(),
            Nan::GetFunction(tpl).ToLocalChecked());
#endif
    }
#ifdef __ENABLE__
private:
    explicit Mp3lameEncoder(
            uint32_t sample_rate,
            uint32_t channel_num,
            uint32_t quality) {
        encoder_ = ttLibC_Mp3lameEncoder_make(sample_rate, channel_num, quality);
    ...
    static NAN_METHOD(New) {
        if(info.IsConstructCall()) {
            if(info.Length() != 3) {
                ...
                Mp3lameEncoder *encoder = new Mp3lameEncoder(
                        info[0]->Uint32Value(),
                        info[1]->Uint32Value(),
                        info[2]->Uint32Value());
    ...
    static bool encodeCallback(void *ptr, ttLibC_Mp3 *mp3) {
        Mp3lameEncoder *encoder = (Mp3lameEncoder *)ptr;
        auto callback = new Nan::Callback(encoder->callback_.As<Function>());
//        Nan::AsyncQueueWorker(new FramePassingWorker((ttLibC_Frame *)mp3, callback));
    ...
    static NAN_METHOD(Encode) {
        ...
        Mp3lameEncoder *encoder = Nan::ObjectWrap::Unwrap<Mp3lameEncoder>(info.Holder());
        ...
        if(frame->type == frameType_pcmS16) {
            if(!ttLibC_Mp3lameEncoder_encode(
                    encoder->encoder_,
                    (ttLibC_PcmS16 *)frame,
                    encodeCallback,
                    encoder)) {
...
NODE_MODULE(mp3lameEncoder, Mp3lameEncoder::Init);
